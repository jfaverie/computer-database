...
<%@ attribute name="offset" type="Integer" required="true"%>
...
<ul class="pagination">
	...
	<c:if test="${currentPage > 0}">
		<li><a href="?page=${currentPage - 1}&nbel=${offset}&colId=${sortCol}&colType=${sortType}&search=${search}"
	...
	<c:choose>
		...
		<c:when test="${currentPage > 2 && currentPage < nbPage - 2}">
			<c:forEach var="cursor" begin="${currentPage - 2}"
				...
				<li ${(cursor == currentPage) ? 'class="active"' : ""}><cdb:link
						classRef="" context="" msg="${cursor}" offset="${offset}"
